...
function positionPopupForMobile() {
  const cdm = document.getElementById('cdm-panel'); // ✅ 너가 붙인 id
  if (!cdm || !popupContent) return;

  const rect = cdm.getBoundingClientRect();
  const margin = 0;         // 컨테이너 양끝에 딱 맞추려면 0
  const topGap = 8;         // 컨테이너 아래/위 여유

  // 팝업을 컨테이너 좌우 폭에 맞춤
  popupContent.style.left = `${rect.left + margin}px`;
  popupContent.style.width = `${rect.width - margin * 2}px`;

  // 세로 위치: 컨테이너 상단 기준으로 띄우거나, 화면 중앙으로 띄워도 됨
  // 여기선 "컨테이너 상단 + 약간"으로
  popupContent.style.top = `${Math.max(8, rect.top + topGap)}px`;

  // 높이도 화면에 맞게 제한
  popupContent.style.maxHeight = `${Math.floor(window.innerHeight * 0.8)}px`;

  // 모바일에서 우측 잘림 방지
  popupContent.style.right = 'auto';
}

function repositionPopup(anchorEl) {
  const isMobile = window.matchMedia('(max-width: 768px)').matches;

  if (isMobile) {
    positionPopupForMobile();
  } else {
    positionPopupNear(anchorEl); // 기존 데스크탑용(카드 근처)
  }
}

popup.classList.remove('hidden'); // 너 방식대로 열기
repositionPopup(clickedCardEl);   // ✅ 위치/폭 결정

window.addEventListener('resize', () => {
  if (!popup.classList.contains('hidden')) {
    repositionPopup(lastPopupAnchor);
  }
});
